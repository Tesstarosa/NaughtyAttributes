...
namespace NaughtyAttributes.Editor
{
	...
	public class DropdownPropertyDrawer : PropertyDrawerBase
	{
				...
				else if (valuesObject is IDropdownList)
				{
					...
					using (var dropdownEnumerator = dropdown.GetEnumerator())
					{
						while (dropdownEnumerator.MoveNext())
						{
							...
							if (current.Value?.Equals(selectedValue) == true) selectedValueIndex = index;
					...
					NaughtyEditorGui.Dropdown(
						rect, property.serializedObject, target, dropdownField, label.text, selectedValueIndex,
						values.ToArray(), displayOptions.ToArray());
				}
			}
			else
			{
		...
		{
			if (values == null || dropdownField == null) return false;
