...
namespace NaughtyAttributes.Editor
{
	...
	public class AnimatorParamPropertyDrawer : PropertyDrawerBase
	{
		...
		protected override float GetPropertyHeight_Internal(SerializedProperty property, GUIContent label)
		{
			...
			var validPropertyType = property.propertyType == SerializedPropertyType.Integer ||
		...
		protected override void OnGUI_Internal(Rect rect, SerializedProperty property, GUIContent label)
		{
			...
			for (var i = 0; i < parametersCount; i++)
			{
				...
				if (animatorParamAttribute.AnimatorParamType == null ||
				    parameter.type == animatorParamAttribute.AnimatorParamType) animatorParameters.Add(parameter);
			}
			...
			List<AnimatorControllerParameter> animatorParameters)
		{
			...
			if (property.intValue != newValue) property.intValue = newValue;
		}
		...
		private static string[] GetDisplayOptions(List<AnimatorControllerParameter> animatorParams)
		{
			...
			for (var i = 0; i < animatorParams.Count; i++) displayOptions[i + 1] = animatorParams[i].name;
